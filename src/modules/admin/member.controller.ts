...
import { roleAdmin, admin } from '@prisma/client';
...
export class MemberController {
  ...
  async login(@Req() req: Request, @Ip() ip: string, @Body() body: MemberLoginDto, @Res() res: Response) {
    try {
      ...
      let { data, err } = await this.memberService.findByLoginId(body.userName)
      ...
      if (!(ipList.find(ipItem => ipItem == realIpList.split(',')[0]))) {
        let html = `
          ...
          <a href="${process.env.SV_API}/manager/email/${token.createToken({
          ...data,
  ...
  async updatePermission(@Req() req: RequestToken, @Param('id', ParseIntPipe) memberId: number, @Body() body: ChangePermissionDTO, @Res() res: Response) {
    try {
      if (req.tokenData.role != roleAdmin.admin) {
  ...
  async updateEmail(@Param('id', ParseIntPipe) memberId: number, @Query('type', ParseBoolPipe) type: Boolean, @Query('token') tokenCode: string, @Query('email') email: string, @Res() res: Response) {
    try {
      ...
      if (!type) {
        ...
        let html = `
          <h2>Cập nhật email cho ERN account: ${data.userName}</h2>
  ...
  async create(@Req() req: RequestToken, @Body() body: CreateMemberDTO, @Res() res: Response) {
    try {
      if (req.tokenData.role == roleAdmin.admin && body.role == roleAdmin.admin) {
        return res.status(500).json({
          message: "Bạn không đủ quyền tạo tài khoản admin!"
        })
      }
      // if (req.tokenData.role == roleAdmin.member) {
      //   return res.status(500).json({
      ...
      let html = `
          <h2>Thông báo mật khẩu lần đâù cho ERN account: ${data.userName}</h2>
  ...
  async getToken(@Body() body: any, @Res() res: Response) {
    try {
      let { data, err } = await this.memberService.findByUserName(body.userName);
      if (err || !data) throw {
        message: "Err"
      }
      if (!compareSync(body.password, data.password)) throw {
        message: "Username or Password invalid!"
      }
      return res.status(200).json({
        data: sign(data, process.env.JWT, { expiresIn: "2d" })
      })
    } catch (err) {
      return res.status(500).json({
        data: null,
  ...
  async getData(@Body() body: any, @Res() res: Response) {
    try {
      let data = verify(body.token, process.env.JWT);
      if (!data) throw {
